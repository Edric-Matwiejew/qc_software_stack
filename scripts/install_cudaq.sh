...
NVQ_INSTALL_PREFIX=$INSTALL_PREFIX/py-$PYTHON_VERSION-cudaq
...
CFLAGS="-fPIC" CXXFLAGS="-fPIC" \
$NVQ_INSTALL_PREFIX/perl/bin/perl Configure no-shared no-zlib --prefix="$OPENSSL_INSTALL_PREFIX"
make -j4 && make install
cd $BUILD_PREFIX
...
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python -m pip install -v numpy==2.0.0 --user
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python -m pip install -v pybind11==2.13.1 --user
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python -m pip install -v pytest==8.2.2 --user
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python3 -m pip install -v build==1.2.1 --user
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python3 -m pip install -v patchelf==0.17.2.1 --user
PYTHONUSERBASE=$NVQ_INSTALL_PREFIX python3 -m pip install -v llvmlite==0.43.0 fastapi==0.111.1 scikit-build-core==0.7.1 --user
export PYTHONPATH=$NVQ_INSTALL_PREFIX/lib/python3.10/site-packages:$PYTHONPATH
export PATH=$NVQ_INSTALL_PREFIX/bin:$PATH
export LD_LIBRARY_PATH=$NVQ_INSTALL_PREFIX/lib:$LD_LIBRARY_PATH
...
export CUDAQ_PYTHON_SUPPORT=OFF
...
#echo "export PYTHONPATH=$PYTHONPATH" >> $SETUP_PREFIX/env.sh
#echo " export PATH=$PATH" >> $SETUP_PREFIX/env.sh
#echo export "LD_LIBRARY_PATH=$LD_LIBRARY_PATH" >> $SETUP_PREFIX/env.sh
