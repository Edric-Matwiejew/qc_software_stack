...
do
	...
	PYTHONPATH=$QISKIT_AER_INSTALL_PREFIX/lib/python${PYTHON_VERSION}/site-packages:$PYTHONPATH
	PATH=$QISKIT_AER_INSTALL_PREFIX/bin:$PATH

	cd $BUILD_PREFIX
	git clone --branch=stable/$QISKIT_AER_VERSION https://github.com/Qiskit/qiskit-aer
	cd qiskit-aer
	rm -rf _skbuild
